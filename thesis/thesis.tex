% Opcje klasy 'iithesis' opisane sa w komentarzach w pliku klasy. Za ich pomocą
% ustawia sie przede wszystkim język i rodzaj (lic/inz/mgr) pracy, oraz czy na
% drugiej stronie pracy ma byc składany wzór oświadczenia o autorskim wykonaniu.
...
do uogólnionych typów algebraicznych zaproponowane niedawno przez
Joshua'e Dunfield'a i Neelakantan'a R. Krishnaswami'ego.
W tej pracy opisujemy nasz język oraz omawiamy nasz wkład w implementację oraz
rozszerzenie systemu typów Dunfield'a i Krishnaswami'ego.
}
\englishabstract{
...
\let\lll\undefined
\usepackage{mathtools, array, amsfonts, mathpartir, amssymb, stmaryrd, cancel, cleveref, fancyvrb}
...
\begin{mathpar}
  \inferrule{\text { A not headed by } \forall / \exists \\
             \text { B not headed by } \forall / \exists \\
             \Gamma \vdash A \equiv B \dashv \Delta}
             {\Gamma \vdash A<:^{\mathcal{P}} B \dashv \Delta} \and
  \inferrule{\text { B not headed by } \forall \\
             \Gamma, \, _{\blacktriangleright \hat{\alpha}}, \, \hat{\alpha} : \kappa
             \vdash [\hat{\alpha} / \alpha] A <:^- B  \dashv
              \Delta, \, _{\blacktriangleright \hat{\alpha}}, \, \Theta}
             {\Gamma \vdash \forall \alpha : \kappa .\, A<:^- B \dashv \Delta} \and
  \inferrule{\Gamma, \, \beta : \kappa \vdash
             A<:^- B \dashv \Delta, \, \beta : \kappa, \, \Theta}
            {\Gamma \vdash A<:^-\forall \beta : \kappa .\, B \dashv \Delta} \and
  \inferrule{\Gamma, \, \alpha : \kappa \vdash
            A<:^+ B \dashv \Delta, \, \alpha : \kappa, \, \Theta}
           {\Gamma \vdash \exists \alpha : \kappa .\, A<:^+ B \dashv \Delta} \and
  \inferrule{\text { A not headed by } \exists \\
             \Gamma, \, _{\blacktriangleright \hat{\beta}}, \, \hat{\beta} : \kappa
             \vdash A<:^+[\hat{\beta} / \beta] B \dashv
             \Delta, \, _{\blacktriangleright \hat{\beta}}, \, \Theta}
            {\Gamma \vdash A<:^+\exists \beta : \kappa .\, B \dashv \Delta} \and
  \inferrule{\Gamma \vdash A <:^- B \dashv \Delta \\ \mathit{neg}(A) \\ \mathit{nonpos}(B)}
            {\Gamma \vdash A <:^+ B \dashv \Delta} \and
  \inferrule{\Gamma \vdash A <:^- B \dashv \Delta \\ \mathit{nonpos}(A) \\ \mathit{neg}(B)}
            {\Gamma \vdash A <:^+ B \dashv \Delta} \and
  \inferrule{\Gamma \vdash A <:^+ B \dashv \Delta \\ \mathit{pos}(A) \\ \mathit{nonneg}(B)}
            {\Gamma \vdash A <:^- B \dashv \Delta} \and
  \inferrule{\Gamma \vdash A <:^+ B \dashv \Delta \\ \mathit{nonneg}(A) \\ \mathit{pos}(B)}
            {\Gamma \vdash A <:^- B \dashv \Delta}
\end{mathpar}
\subsubsection*{Typing}
$e \: \textit{chk-I}$ -- expression is a checked introduction form.
\begin{mathpar}
  \inferrule{\\}{()\: \textit{chk-I}} \and
  \inferrule{\\}{\lambda x. \, e \: \textit{chk-I}} \and
  \inferrule{\\}{(e_1, e_2) \: \textit{chk-I}}
\end{mathpar}
\SaveVerb{unit}+()+
\SaveVerb{case}+case+
...
\begin{mathpar}
  \inferrule{(x : A \: p) \in \Gamma}{\Gamma \vdash x \Rightarrow[\Gamma] A \: p \dashv \Gamma} \and
  \inferrule{\Gamma \vdash e \Rightarrow A \: q \dashv \Theta \\
             \Theta \vdash A<: ^ {\text { join }(\text { pol }(B), \text { pol }(A)) } B \dashv \Delta}
             {\Gamma \vdash e \Leftarrow B \: p \dashv \Delta} \and
  \inferrule{\Gamma \vdash A \: \textbf{!} \: \mathit{type} \\
             \Gamma \vdash e \Leftarrow [\Gamma]A \: \textbf{!} \dashv \Delta}
             {\Gamma \vdash (e : A) \Rightarrow [\Delta] A \: \textbf{!} \dashv \Delta} \and
  \inferrule{\\}{\Gamma \vdash () \Leftarrow \UseVerb{unit} \: p \dashv \Gamma} \and
  \inferrule{\\}{\Gamma[\hat{\alpha} : \star] \vdash () \Leftarrow \hat{\alpha}
                 \dashv \Gamma[\hat{\alpha} : \star=\UseVerb{unit}]} \and
  \inferrule{v \: \textit{chk-I} \\ \Gamma, \, \alpha : \kappa \vdash v \Leftarrow A \: p
             \dashv \Delta, \, \alpha : \kappa, \, \Theta}
            {\Gamma \vdash v \Leftarrow \forall \alpha : \kappa. \, A \: p \dashv \Delta} \and
  \inferrule{e \: \textit{chk-I} \\ \Gamma, \, \hat{\alpha} : \kappa
             \vdash e \Leftarrow [\hat{\alpha} / \alpha] A \dashv \Delta}
             {\Gamma \vdash e \Leftarrow \exists \alpha : \kappa. \, A \:p \dashv \Delta} \and
  \inferrule{v \: \textit{chk-I} \\ \Gamma, \, _{\blacktriangleright P} / P \dashv \Theta \\
             \Theta \vdash v \Leftarrow [\Theta]A \: \textbf{!}
             \dashv \Delta , \, _{\blacktriangleright P}, \, \Delta'}
             {\Gamma \vdash v \Leftarrow P \supset A \: \textbf{!} \dashv \Delta} \and
  \inferrule{v \: \textit{chk-I} \\ \Gamma, \, _{\blacktriangleright P} / P \dashv \bot}
             {\Gamma \vdash v \Leftarrow P \supset A \: \textbf{!} \dashv \Gamma} \and
  \inferrule{\text{$e$ not a \UseVerb{case}} \\ \Gamma \vdash P \: \mathit{true} \dashv \Theta \\
             \Theta \vdash e \Leftarrow [\Theta]A \: p \dashv \Delta}
             {\Gamma \vdash e \Leftarrow A \wedge P \: p \dashv \Delta} \and
  \inferrule{\Gamma, \, x : A \: p \vdash e \Leftarrow B \: p \dashv
             \Delta, \, x : A \: p, \, \Theta}
             {\Gamma \vdash \lambda x. \, e \Leftarrow A \rightarrow B \: p \dashv \Delta} \and
  \inferrule{\Gamma\left[\hat{\alpha}_{1} : \star, \hat{\alpha}_{2} : \star, \hat{\alpha} : \star=\hat{\alpha}_{1}
             \rightarrow \hat{\alpha}_{2}\right], x : \hat{\alpha}_{1} \vdash e \Leftarrow \hat{\alpha}_{2} \dashv
             \Delta, \, x : \hat{\alpha}_{1}, \, \Delta'}
             {\Gamma[\hat{\alpha} : \star] \vdash \lambda x. \, e \Leftarrow \hat{\alpha} \dashv \Delta} \and
  \inferrule{\Gamma \vdash e \Rightarrow A \: p \dashv \Theta \\
             \Theta \vdash s : A \: p \gg C\lceil q\rceil \dashv \Delta}
             {\Gamma \vdash e \: s \Rightarrow C \: q \dashv \Delta} \and
  \inferrule{\Gamma \vdash e_1 \Leftarrow A_1 \: p \dashv \Theta \\
             \Theta \vdash e_2 \Leftarrow [\Theta]A_2 \: p \dashv \Delta}
            {\Gamma \vdash (e_1, e_2) \Leftarrow A_1 \times A_2 \: p \dashv \Delta} \and
  \inferrule{\Gamma\left[\hat{\alpha}_{2} : \star, \hat{\alpha}_{1} : \star,
             \hat{\alpha} : \star=\hat{\alpha}_{1} \times \hat{\alpha}_{2}\right]
             \vdash e_{1} \Leftarrow \hat{\alpha}_{1} \dashv \Theta \\
             \Theta \vdash e_{2} \Leftarrow[\Theta] \hat{\alpha}_{2} \dashv \Delta}
            {\Gamma[\hat{\alpha} : \star] \vdash (e_1, e_2) \Leftarrow \hat{\alpha} \dashv \Delta}
\end{mathpar}
\vspace{1cm}
\subsubsection*{Spine typing}
$\Gamma \vdash s : A \: p \gg C\lceil q\rceil \dashv \Delta$ -- under the context $\Gamma$,
passing spine $s$ to a function of type $A$ synthesizes type $C$; recover principality if possible.
\vspace{0.2cm}
\begin{mathpar}
  \inferrule{\Gamma, \, \hat{\alpha} : \kappa \vdash e \; s :
            [\hat{\alpha} / \alpha]A \gg C \: q \dashv \Delta}
            {\Gamma \vdash e \; s : \forall \alpha : \kappa. \, A \: p \gg C \: q \dashv \Delta} \and
  \inferrule{\Gamma \vdash P \: \mathit{true} \dashv \Theta \;\;\;\;
             \Theta \vdash e \; s : [\Theta]A \: p \gg C \: q \dashv \Delta}
            {\Gamma \vdash e \; s : P \supset A \: p \gg C \: q \dashv \Delta} \and
  \inferrule{\\}{\Gamma \vdash \cdot : A \: p \gg A \: p\: \dashv \Gamma} \and
  \inferrule{\Gamma \vdash e \Leftarrow A \: p \dashv \Theta \\
             \Theta \vdash s : [\Theta]B \: p \gg C \: q \dashv \Delta}
            {\Gamma \vdash e \; s : A \rightarrow B \: p \gg C \: q \dashv \Delta} \and
  \inferrule{\Gamma \left[\hat{\alpha}_{2} : \star, \hat{\alpha}_{1} : \star, \hat{\alpha} : \star=\hat{\alpha}_{1}
            \rightarrow \hat{\alpha}_{2}\right] \vdash e \; s :
            (\hat{\alpha}_{1} \rightarrow \hat{\alpha}_{2}) \gg C \dashv \Delta}
            {\Gamma [\hat{\alpha} : \star] \vdash e \; s : \hat{\alpha} \gg C \dashv \Delta} \and
  \inferrule{\Gamma \vdash s : A \: \textbf{!} \gg C \cancel{\textbf{!}} \dashv \Delta \\
            \mathit{FEV}(C) = \emptyset}
            {\Gamma \vdash s : A \: \textbf{!} \gg C\lceil \textbf{!}\rceil \dashv \Delta} \and
  \inferrule{\Gamma \vdash s : A \: p \gg C \: q \dashv \Delta \\
             \big((p = \cancel{\textbf{!}}) \operatorname{or} \,
             (q = \textbf{!}) \operatorname{or} \, (\mathit{FEV}(C) \neq \emptyset)\big)}
            {\Gamma \vdash s : A \: p \gg C\lceil q\rceil \dashv \Delta}
\end{mathpar}
...
\begin{mathpar}
\inferrule{\Gamma \vdash A' <:^+ A \dashv \Theta \\
           \Theta \vdash [\Theta]B <:^- [\Theta]B' \dashv \Delta}
...
\begin{mathpar}
  \inferrule{\text{$B$ not guarded} \\ \Gamma \vdash P \: \mathit{true} \dashv \Theta \\
            \Theta \vdash [\Theta]A <:^- [\Theta]B \dashv \Delta}
            {\Gamma \vdash P \supset A <:^- B \dashv \Delta} \and
  \inferrule{\Gamma, \, _{\blacktriangleright P} / P \dashv \Theta \\
            \Theta \vdash [\Theta]A <:^- [\Theta]B \dashv \Delta , \, _{\blacktriangleright P}, \, \Delta'}
            {\Gamma \vdash A <:^- P \supset B \dashv \Delta} \and
  \inferrule{\Gamma, \, _{\blacktriangleright P} / P \dashv \bot}
            {\Gamma \vdash A <:^- P \supset B \dashv \Gamma} \and
  \inferrule{\Gamma, \, _{\blacktriangleright P} / P \dashv \Theta \\
            \Theta \vdash [\Theta]A <:^+ [\Theta]B \dashv \Delta , \, _{\blacktriangleright P}, \, \Delta'}
            {\Gamma \vdash A \wedge P <:^+ B \dashv \Delta} \and
  \inferrule{\Gamma, \, _{\blacktriangleright P} / P \dashv \bot}
            {\Gamma \vdash A \wedge P <:^+ B \dashv \Gamma} \and
  \inferrule{\text{$A$ not asserting} \\ \Gamma \vdash P \: \mathit{true} \dashv \Theta \\
            \Theta \vdash [\Theta]A <:^+ [\Theta]B \dashv \Delta}
            {\Gamma \vdash A <:^+ B \wedge P \dashv \Delta}
\end{mathpar}
...
\end{verbatim}
It is true that we can define \verb+Vec+ like this, but there is a drawback to that approach. Since we use
...
prnc(A, \; p) = \begin{cases}
  \cancel{\textbf{!}} & when \; \mathit{FEV}(A) \neq \emptyset \\
  p                   & when \; \mathit{FEV}(A) = \emptyset
\end{cases}
